...
class IsolationTests(unittest.TestCase):
    ...
    def test_06_lease_seq_and_upstream_url_guards(self):
        b=self.b;a=self.a;b.create(a,self.c);_,r=b.poll(a,self.c);t=r['task'];url='https://chatgpt.com/c/'+str(uuid.uuid4());provisional='https://chatgpt.com/c/WEB:'+str(uuid.uuid4())
        ...
        self.assertEqual(b.event(a,t,2,'checkpoint',checkpoint={'url':provisional})[0],200)
        self.assertEqual(b.call('GET','/api/conversations/'+self.c['id'],token=a['token'])[1]['upstreamUrl'],'')
        self.assertEqual(b.event(a,t,3,'snapshot',text='text once',checkpoint={'url':url})[0],200)
        self.assertTrue(b.event(a,t,3,'snapshot',text='different duplicate')[1]['duplicate'])
        self.assertEqual(b.call('GET','/api/tasks/'+t['id'],token=a['token'])[1]['text'],'text once')
        self.assertEqual(b.event(a,t,4,'snapshot',text='wrong context',checkpoint={'url':'https://chatgpt.com/c/another'})[0],409)
        self.assertEqual(b.event(a,t,4,'done',text='final A',checkpoint={'url':url})[0],200)
        self.assertEqual(b.call('GET','/api/conversations/'+self.c['id'],token=a['token'])[1]['upstreamUrl'],url)
        self.assertEqual(b.event(a,t,5,'snapshot',text='late',checkpoint={'url':'https://chatgpt.com/c/not-this'})[0],200)
