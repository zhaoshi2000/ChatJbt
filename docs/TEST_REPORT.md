...
先以两个独立浏览器配置文件安装本包扩展，A/B 各创建并配对专属令牌，手动登录对应上游账号。A 窗口开两个本地新会话，B 窗口开一个。分别发不同问题，核对 A 的两个会话按顺序复用同一工作标签页，B 使用自己的工作标签页，历史和回答归属正确。然后分别测试普通网页刷新、同会话另页查看、取消 A 而 B 继续、关闭工作页后自动恢复、停止后端再启动。
