...
function applyUrl(){const u=new URL(location.href);u.search='';if(selected)u.searchParams.set('c',selected);history.replaceState(null,'',u.pathname+u.search);document.title=(current?.title||'新对话')+' · '+(account?.name||'GBT');}
...
async function shareReply(task){const text=task.text||'';if(navigator.share)try{await navigator.share({title:current?.title||'GBT 回复',text});return;}catch(error){if(error.name==='AbortError')return;}await copyText(text);toast('回复已复制，可以粘贴分享');}
...
function turnElement(task){let row=document.getElementById('turn-'+task.id);if(row)return row;row=document.createElement('article');row.id='turn-'+task.id;row.className='turn';
  ...
  const head=document.createElement('div');head.className='assistant-head';const icon=document.createElement('img');icon.src='avatar.png';icon.alt='GBT 头像';const name=document.createElement('span');name.textContent='GBT';const tag=document.createElement('span');tag.className='state-tag';head.append(icon,name,tag);
  ...
  menu.className='turn-menu';menu.hidden=true;const edit=document.createElement('button');edit.type='button';edit.textContent='编辑后再次提问';edit.addEventListener('click',()=>{menu.hidden=true;$('input').value=latest().message;saveDraft();autosize();$('input').focus();});const save=document.createElement('button');save.type='button';save.textContent='导出本条 Markdown';save.addEventListener('click',()=>{menu.hidden=true;download('GBT-回复-'+task.id+'.md',taskMarkdown(latest()),'text/markdown;charset=utf-8');});menu.append(edit,save);const feedback=localStorage.getItem('doubao.v12.feedback:'+task.id);up.setAttribute('aria-pressed',String(feedback==='up'));down.setAttribute('aria-pressed',String(feedback==='down'));actions.append(copy,up,down,share,retry,more,menu);row.append(user,head,answer,detail,actions);return row;
}
