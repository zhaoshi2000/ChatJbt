...
let account=null,provider='browser',conversations=[],summaries=[],tasks=[],current=null,epoch=0,refreshing=false,sending=false,mode='chat',toastTimer,attachments=[];
...
function renderHistory(){const q=$('search').value.trim().toLowerCase(),items=conversations.filter(c=>c.title.toLowerCase().includes(q));$('conversationCount').textContent=String(conversations.length);$('history').replaceChildren();
  for(const c of items){const button=document.createElement('button');button.className='history-item'+(c.id===selected?' active':'');button.dataset.id=c.id;const label=document.createElement('span');label.className='label';label.textContent=c.title;button.append(label);if(summaries.some(t=>t.conversationId===c.id&&!TERMINAL.has(t.state))){const dot=document.createElement('span');dot.className='running-dot';dot.title='正在生成';button.append(dot);}button.addEventListener('click',()=>select(c.id));$('history').append(button);}
...
function turnElement(task){let row=document.getElementById('turn-'+task.id);if(row)return row;row=document.createElement('article');row.id='turn-'+task.id;row.className='turn';
  ...
  const head=document.createElement('div');head.className='assistant-head';const icon=document.createElement('img');icon.src='icon.svg';icon.alt='';const name=document.createElement('span');name.textContent='逗包';const tag=document.createElement('span');tag.className='state-tag';head.append(icon,name,tag);
  ...
  const copy=document.createElement('button');copy.textContent='复制回复';copy.addEventListener('click',()=>copyText(tasks.find(t=>t.id===task.id)?.text||''));const retry=document.createElement('button');retry.textContent='再次使用消息';retry.addEventListener('click',()=>{if(pending()){toast('请先确认上次发送结果');return;}$('input').value=task.message;saveDraft();autosize();$('input').focus();});actions.append(copy,retry);row.append(user,head,answer,detail,actions);return row;
}
function renderTasks(forceBottom=false){const box=$('conversation'),stick=forceBottom||box.scrollHeight-box.scrollTop-box.clientHeight<100;const ids=new Set(tasks.map(t=>'turn-'+t.id));
  ...
  for(const task of tasks){const row=turnElement(task),answer=row.querySelector('.answer');
    if(row.dataset.version!==String(task.version)||row.dataset.state!==task.state){
      const signature=task.text+'|'+JSON.stringify((task.images||[]).map(x=>[x.name,x.base64?.length]));if(answer._content!==signature||(!task.text&&!task.images?.length&&row.dataset.state!==task.state)){answer._content=signature;if(task.text)renderMarkdown(answer,task.text);else answer.replaceChildren();if(task.images?.length){const images=document.createElement('div');images.className='message-images';for(const [index,a] of task.images.entries()){if(!a.base64)continue;const link=document.createElement('a');link.href=`data:${a.mimeType};base64,${a.base64}`;link.download=a.name||`生成图片-${index+1}`;link.title='点击下载生成图片';const img=document.createElement('img');img.src=link.href;img.alt=a.name||'生成图片';link.append(img);images.append(link);}answer.append(images);}if(!task.text&&!task.images?.length){const el=document.createElement('span');el.className='empty';el.textContent=task.state==='queued'?'任务已进入本账号队列，等待此会话工作页接入…':TERMINAL.has(task.state)?'本次没有正文或图片输出。':'正在等待本会话的回复…';answer.append(el);}}
      row.querySelector('.state-tag').textContent=stateLabel(task.state);row.querySelector('.task-detail').textContent=task.detail||'';row.dataset.version=String(task.version);row.dataset.state=task.state;
    }$('messages').append(row);
...
function conversationMarkdown(){return '# '+(current?.title||'逗包会话')+'\n\n账号：'+(account?.name||'')+'\n会话 ID：'+selected+'\n\n'+tasks.map(t=>taskMarkdown(t)).join('\n\n---\n\n');}
...
$('send').addEventListener('click',send);$('input').addEventListener('input',()=>{saveDraft();autosize();});$('input').addEventListener('keydown',e=>{if(!e.isComposing&&e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();send();}});
...
$('conversationMenu').addEventListener('click',()=>{if(!current){toast('开始对话后可重命名、导出和删除会话');return;}$('renameTitle').value=current.title;showDialog('conversationDialog');});
$('rename').addEventListener('click',async()=>{try{await apiRequest(config,'/api/conversations/'+selected,{method:'POST',body:{title:$('renameTitle').value}});$('conversationDialog').close();await refresh(true);}catch(e){toast(e.message);}});
$('exportConversation').addEventListener('click',()=>download('逗包-'+selected+'.md',conversationMarkdown(),'text/markdown;charset=utf-8'));
$('deleteConversation').addEventListener('click',async()=>{if(!confirm('删除此本地会话及本地消息？不会删除上游网页记录。'))return;try{await apiRequest(config,'/api/conversations/'+selected,{method:'DELETE'});$('conversationDialog').close();await select('');}catch(e){toast(e.message);}});
