...
function renderTasks(forceBottom=false){const box=$('conversation'),stick=forceBottom||box.scrollHeight-box.scrollTop-box.clientHeight<100;const ids=new Set(tasks.map(t=>'turn-'+t.id));
  ...
  for(const task of tasks){const row=turnElement(task),answer=row.querySelector('.answer');
    if(row.dataset.version!==String(task.version)||row.dataset.state!==task.state){
      const signature=task.text+'|'+JSON.stringify([(task.images||[]).map(x=>[x.name,x.base64?.length]),(task.files||[]).map(x=>[x.id,x.name,x.size])]);if(answer._content!==signature||(!task.text&&!task.images?.length&&!task.files?.length&&row.dataset.state!==task.state)){answer._content=signature;if(task.text)renderMarkdown(answer,task.text);else answer.replaceChildren();if(task.images?.length){const images=document.createElement('div');images.className='message-images';for(const [index,a] of task.images.entries()){if(!a.base64)continue;const link=document.createElement('a');link.href=`data:${a.mimeType};base64,${a.base64}`;link.download=a.name||`生成图片-${index+1}`;link.title='点击下载生成图片';const img=document.createElement('img');img.src=link.href;img.alt=a.name||'生成图片';link.append(img);images.append(link);}answer.append(images);}if(task.files?.length){const files=document.createElement('div');files.className='generated-files';for(const file of task.files){const button=document.createElement('button');button.type='button';button.className='generated-file';button.innerHTML='<span aria-hidden="true">⇩</span><span></span><small></small>';button.children[1].textContent=file.name;button.querySelector('small').textContent=formatBytes(file.size);button.addEventListener('click',()=>downloadTaskFile(task,file,button));files.append(button);}answer.append(files);}if(!task.text&&!task.images?.length&&!task.files?.length){const el=document.createElement('span');el.className='empty';el.textContent=task.state==='queued'?'任务已进入本账号队列，等待此会话工作页接入…':TERMINAL.has(task.state)?'本次没有正文、图片或文件输出。':'正在等待本会话的回复…';answer.append(el);}}
      row.querySelector('.state-tag').textContent=stateLabel(task.state);row.querySelector('.task-detail').textContent=task.detail||'';row.querySelector('.turn-actions').hidden=!TERMINAL.has(task.state)||(!task.text&&!task.images?.length&&!task.files?.length);row.dataset.version=String(task.version);row.dataset.state=task.state;
    }$('messages').append(row);
