...
function openConversationDialog(id){const c=conversations.find(x=>x.id===id);if(!c)return;actionConversationId=id;$('renameTitle').value=c.title;$('pinConversation').textContent=c.pinned?'取消置顶':'置顶';showDialog('conversationDialog');}
function historyMenuButton(label,icon,handler,danger=false){const button=document.createElement('button');button.type='button';button.innerHTML=`<span aria-hidden="true">${icon}</span><span>${label}</span>`;if(danger)button.classList.add('danger');button.addEventListener('click',event=>{event.stopPropagation();closeHistoryMenu();handler();});return button;}
...
async function deleteConversation(id){const c=conversations.find(x=>x.id===id);if(!confirm('删除“'+(c?.title||'此会话')+'”及本地消息？不会删除 ChatGPT 上的对话。'))return;try{await apiRequest(config,'/api/conversations/'+id,{method:'DELETE'});if($('conversationDialog').open)$('conversationDialog').close();if(id===selected)await select('');else await refresh(true);}catch(error){toast(error.message);}}
...
$('deleteConversation').addEventListener('click',()=>deleteConversation(actionConversationId||selected));
